...
SourceWriter& SourceWriter::BeginBlock() {
...
SourceWriter& SourceWriter::BeginMethod(const Method& method, int modifiers) {
  // Start a new generic namespace, looking for new types to declare at this
  // method level
...
SourceWriter& SourceWriter::BeginClass(const Type& clazz,
    const std::vector<Type>* dependencies, int modifiers) {
  if (!clazz.package().empty()) {
    Append("package ").Append(clazz.package()).Append(";").EndLine();
  }
  if (dependencies != nullptr && !dependencies->empty()) {
    ...
    for (std::set<Type, Type::Comparator>::const_iterator it = imports.cbegin();
        it != imports.cend(); ++it) {
      Append("import ").Append(it->package()).Append(".").Append(it->name())
          .Append(";").EndLine();
    }
  }
  return BeginClass(clazz, modifiers);
}

SourceWriter& SourceWriter::BeginClass(const Type& clazz, int modifiers) {
  // Start a new generic namespace, looking for new types to declare at this
  // class level
  GenericNamespace* generic_namespace = PushGenericNamespace(modifiers);
  VisitType(clazz, generic_namespace);

  EndLine();
  WriteDoc(clazz.description());
  if (!clazz.annotations().empty()) {
    WriteAnnotations(clazz.annotations());
  }
  WriteModifiers(modifiers).Append("class ").Append(clazz.name());
...
SourceWriter& SourceWriter::EndClass() {
  EndBlock();
  PopGenericNamespace();
  return *this;
}
...
SourceWriter& SourceWriter::WriteAnnotations(const std::vector<Annotation>& annotations) {
...
SourceWriter& SourceWriter::WriteGenerics(const std::vector<const Type*>& generics) {
  Append("<");
...
SourceWriter::GenericNamespace* SourceWriter::PushGenericNamespace(int modifiers) {
