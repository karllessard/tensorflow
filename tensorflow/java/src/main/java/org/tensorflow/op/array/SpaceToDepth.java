...
public final class SpaceToDepth<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> SpaceToDepth<T> create(Scope scope, Operand<T> input, Integer blockSize) {
   ...
   * @return a new instance of SpaceToDepth
   **/
  public static <T> SpaceToDepth<T> create(Scope scope, Operand<T> input, Integer blockSize, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("SpaceToDepth", scope.makeOpName("SpaceToDepth"));
    opBuilder.addInput(input.asOutput());
    opBuilder.setAttr("blockSize", blockSize);
