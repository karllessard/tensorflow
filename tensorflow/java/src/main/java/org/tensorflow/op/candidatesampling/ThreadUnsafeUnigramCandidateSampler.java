...
public final class ThreadUnsafeUnigramCandidateSampler extends PrimitiveOp {
  ...
  /**
   * Factory method to create a class to wrap a new ThreadUnsafeUnigramCandidateSampler operation to the graph.
   * 
   ...
   **/
  public static ThreadUnsafeUnigramCandidateSampler create(Scope scope, Operand<Long> trueClasses, Integer numTrue, Integer numSampled, Boolean unique, Integer rangeMax) {
    OperationBuilder opBuilder = scope.graph().opBuilder("ThreadUnsafeUnigramCandidateSampler", scope.makeOpName("ThreadUnsafeUnigramCandidateSampler"));
    ...
    return new ThreadUnsafeUnigramCandidateSampler(opBuilder.build());
  }
   ...
   * @return a new instance of ThreadUnsafeUnigramCandidateSampler
   **/
  public static ThreadUnsafeUnigramCandidateSampler create(Scope scope, Operand<Long> trueClasses, Integer numTrue, Integer numSampled, Boolean unique, Integer rangeMax, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("ThreadUnsafeUnigramCandidateSampler", scope.makeOpName("ThreadUnsafeUnigramCandidateSampler"));
    opBuilder.addInput(trueClasses.asOutput());
    opBuilder.setAttr("numTrue", numTrue);
    opBuilder.setAttr("numSampled", numSampled);
  ...
  private ThreadUnsafeUnigramCandidateSampler(Operation operation) {
    super(operation);
    int outputIdx = 0;
