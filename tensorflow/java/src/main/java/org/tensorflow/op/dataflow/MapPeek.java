...
public final class MapPeek extends PrimitiveOp implements Iterable<Operand<DataType>> {
   ...
   **/
  public static MapPeek create(Scope scope, Operand<Long> key, Operand<Integer> indices, List<DataType> dtypes) {
   ...
   * @return a new instance of MapPeek
   **/
  public static MapPeek create(Scope scope, Operand<Long> key, Operand<Integer> indices, List<DataType> dtypes, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("MapPeek", scope.makeOpName("MapPeek"));
    opBuilder.addInput(key.asOutput());
    opBuilder.addInput(indices.asOutput());
    opBuilder.setAttr("dtypes", dtypes.toArray(new DataType[dtypes.size()]));
