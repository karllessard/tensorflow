...
public final class MapUnstage extends PrimitiveOp implements Iterable<Operand<DataType>> {
   ...
   **/
  public static MapUnstage create(Scope scope, Operand<Long> key, Operand<Integer> indices, List<DataType> dtypes) {
   ...
   * @return a new instance of MapUnstage
   **/
  public static MapUnstage create(Scope scope, Operand<Long> key, Operand<Integer> indices, List<DataType> dtypes, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("MapUnstage", scope.makeOpName("MapUnstage"));
    opBuilder.addInput(key.asOutput());
    opBuilder.addInput(indices.asOutput());
    opBuilder.setAttr("dtypes", dtypes.toArray(new DataType[dtypes.size()]));
