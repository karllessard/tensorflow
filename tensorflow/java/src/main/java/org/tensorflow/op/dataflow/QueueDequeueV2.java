...
public final class QueueDequeueV2 extends PrimitiveOp implements Iterable<Operand<DataType>> {
  ...
  /**
   * Factory method to create a class to wrap a new QueueDequeueV2 operation to the graph.
   * 
   * @param scope Current graph scope
   * @param handle
   * @param componentTypes
   * @return a new instance of QueueDequeueV2
   **/
  public static QueueDequeueV2 create(Scope scope, Operand<?> handle, List<DataType> componentTypes) {
    OperationBuilder opBuilder = scope.graph().opBuilder("QueueDequeueV2", scope.makeOpName("QueueDequeueV2"));
    opBuilder.addInput(handle.asOutput());
    opBuilder.setAttr("componentTypes", componentTypes.toArray(new DataType[componentTypes.size()]));
    return new QueueDequeueV2(opBuilder.build());
  }
   ...
   * @return a new instance of QueueDequeueV2
   **/
  public static QueueDequeueV2 create(Scope scope, Operand<?> handle, List<DataType> componentTypes, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("QueueDequeueV2", scope.makeOpName("QueueDequeueV2"));
    opBuilder.addInput(handle.asOutput());
    opBuilder.setAttr("componentTypes", componentTypes.toArray(new DataType[componentTypes.size()]));
  ...
  private QueueDequeueV2(Operation operation) {
    super(operation);
    int outputIdx = 0;
