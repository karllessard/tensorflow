...
public final class TensorArrayConcat<T> extends PrimitiveOp {
   ...
   **/
  public static <T> TensorArrayConcat<T> create(Scope scope, Operand<String> handle, Operand<Float> flowIn, Class<T> dtype) {
    OperationBuilder opBuilder = scope.graph().opBuilder("TensorArrayConcat", scope.makeOpName("TensorArrayConcat"));
   ...
   * @return a new instance of TensorArrayConcat
   **/
  public static <T> TensorArrayConcat<T> create(Scope scope, Operand<String> handle, Operand<Float> flowIn, Class<T> dtype, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("TensorArrayConcat", scope.makeOpName("TensorArrayConcat"));
    opBuilder.addInput(handle.asOutput());
    opBuilder.addInput(flowIn.asOutput());
    opBuilder.setAttr("dtype", DataType.fromClass(dtype));
