...
public final class TensorArrayGather<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> TensorArrayGather<T> create(Scope scope, Operand<String> handle, Operand<Integer> indices, Operand<Float> flowIn, Class<T> dtype) {
    OperationBuilder opBuilder = scope.graph().opBuilder("TensorArrayGather", scope.makeOpName("TensorArrayGather"));
   ...
   * @return a new instance of TensorArrayGather
   **/
  public static <T> TensorArrayGather<T> create(Scope scope, Operand<String> handle, Operand<Integer> indices, Operand<Float> flowIn, Class<T> dtype, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("TensorArrayGather", scope.makeOpName("TensorArrayGather"));
    opBuilder.addInput(handle.asOutput());
    opBuilder.addInput(indices.asOutput());
    opBuilder.addInput(flowIn.asOutput());
