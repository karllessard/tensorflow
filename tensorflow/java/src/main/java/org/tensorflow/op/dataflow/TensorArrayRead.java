...
public final class TensorArrayRead<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> TensorArrayRead<T> create(Scope scope, Operand<String> handle, Operand<Integer> index, Operand<Float> flowIn, Class<T> dtype) {
    OperationBuilder opBuilder = scope.graph().opBuilder("TensorArrayRead", scope.makeOpName("TensorArrayRead"));
