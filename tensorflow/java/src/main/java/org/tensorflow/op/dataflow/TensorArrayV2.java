...
public final class TensorArrayV2 extends PrimitiveOp implements Operand<String> {
  ...
  /**
   * Factory method to create a class to wrap a new TensorArrayV2 operation to the graph.
   * 
   * @param scope Current graph scope
   * @param size
   * @param dtype
   * @return a new instance of TensorArrayV2
   **/
  public static <T> TensorArrayV2 create(Scope scope, Operand<Integer> size, Class<T> dtype) {
    OperationBuilder opBuilder = scope.graph().opBuilder("TensorArrayV2", scope.makeOpName("TensorArrayV2"));
    opBuilder.addInput(size.asOutput());
    opBuilder.setAttr("dtype", DataType.fromClass(dtype));
    return new TensorArrayV2(opBuilder.build());
  }
   ...
   * @return a new instance of TensorArrayV2
   **/
  public static <T> TensorArrayV2 create(Scope scope, Operand<Integer> size, Class<T> dtype, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("TensorArrayV2", scope.makeOpName("TensorArrayV2"));
    opBuilder.addInput(size.asOutput());
    opBuilder.setAttr("dtype", DataType.fromClass(dtype));
  ...
  private TensorArrayV2(Operation operation) {
    super(operation);
    int outputIdx = 0;
