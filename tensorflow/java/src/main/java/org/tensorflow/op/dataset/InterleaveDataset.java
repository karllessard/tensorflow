...
public final class InterleaveDataset extends PrimitiveOp implements Operand<Object> {
  
  /**
   * Factory method to create a class to wrap a new InterleaveDataset operation to the graph.
   * 
   ...
   **/
  public static InterleaveDataset create(Scope scope, Operand<?> inputDataset, Iterable<Operand<?>> otherArguments, Operand<Long> cycleLength, Operand<Long> blockLength, Object f, List<DataType> outputTypes, List<Shape> outputShapes) {
    OperationBuilder opBuilder = scope.graph().opBuilder("InterleaveDataset", scope.makeOpName("InterleaveDataset"));
