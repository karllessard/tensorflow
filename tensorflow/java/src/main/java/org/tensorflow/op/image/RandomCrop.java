...
public final class RandomCrop<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> RandomCrop<T> create(Scope scope, Operand<T> image, Operand<Long> size) {
   ...
   * @return a new instance of RandomCrop
   **/
  public static <T> RandomCrop<T> create(Scope scope, Operand<T> image, Operand<Long> size, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("RandomCrop", scope.makeOpName("RandomCrop"));
    opBuilder.addInput(image.asOutput());
    opBuilder.addInput(size.asOutput());
