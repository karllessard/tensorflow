...
public final class SparseMatMul extends PrimitiveOp implements Operand<Float> {
   ...
   **/
  public static <T, U> SparseMatMul create(Scope scope, Operand<T> a, Operand<U> b) {
   ...
   * @return a new instance of SparseMatMul
   **/
  public static <T, U> SparseMatMul create(Scope scope, Operand<T> a, Operand<U> b, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("SparseMatMul", scope.makeOpName("SparseMatMul"));
    opBuilder.addInput(a.asOutput());
    opBuilder.addInput(b.asOutput());
    ...
    if (options.aIsSparse != null) {
      opBuilder.setAttr("aIsSparse", options.aIsSparse);
    }
    if (options.bIsSparse != null) {
      opBuilder.setAttr("bIsSparse", options.bIsSparse);
    }
    return new SparseMatMul(opBuilder.build());
  }
