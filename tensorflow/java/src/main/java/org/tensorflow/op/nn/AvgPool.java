...
public final class AvgPool<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> AvgPool<T> create(Scope scope, Operand<T> value, List<Integer> ksize, List<Integer> strides, String padding) {
   ...
   * @return a new instance of AvgPool
   **/
  public static <T> AvgPool<T> create(Scope scope, Operand<T> value, List<Integer> ksize, List<Integer> strides, String padding, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("AvgPool", scope.makeOpName("AvgPool"));
    opBuilder.addInput(value.asOutput());
    long[] ksizeArray = new long[ksize.size()];
    for (int i = 0; i < ksizeArray.length; ++i) {
