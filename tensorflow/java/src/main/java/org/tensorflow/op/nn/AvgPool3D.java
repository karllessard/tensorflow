...
public final class AvgPool3D<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> AvgPool3D<T> create(Scope scope, Operand<T> input, List<Integer> ksize, List<Integer> strides, String padding) {
   ...
   * @return a new instance of AvgPool3D
   **/
  public static <T> AvgPool3D<T> create(Scope scope, Operand<T> input, List<Integer> ksize, List<Integer> strides, String padding, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("AvgPool3D", scope.makeOpName("AvgPool3D"));
    opBuilder.addInput(input.asOutput());
    long[] ksizeArray = new long[ksize.size()];
    for (int i = 0; i < ksizeArray.length; ++i) {
