...
public final class Conv3D<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> Conv3D<T> create(Scope scope, Operand<T> input, Operand<T> filter, List<Integer> strides, String padding) {
   ...
   * @return a new instance of Conv3D
   **/
  public static <T> Conv3D<T> create(Scope scope, Operand<T> input, Operand<T> filter, List<Integer> strides, String padding, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("Conv3D", scope.makeOpName("Conv3D"));
    opBuilder.addInput(input.asOutput());
    opBuilder.addInput(filter.asOutput());
    long[] stridesArray = new long[strides.size()];
