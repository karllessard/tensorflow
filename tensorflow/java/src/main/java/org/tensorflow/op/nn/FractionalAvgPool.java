...
public final class FractionalAvgPool<T> extends PrimitiveOp {
   ...
   **/
  public static <T> FractionalAvgPool<T> create(Scope scope, Operand<T> value, List<Float> poolingRatio) {
   ...
   * @return a new instance of FractionalAvgPool
   **/
  public static <T> FractionalAvgPool<T> create(Scope scope, Operand<T> value, List<Float> poolingRatio, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("FractionalAvgPool", scope.makeOpName("FractionalAvgPool"));
    opBuilder.addInput(value.asOutput());
    float[] poolingRatioArray = new float[poolingRatio.size()];
    for (int i = 0; i < poolingRatioArray.length; ++i) {
