...
public final class MaxPool3D<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T> MaxPool3D<T> create(Scope scope, Operand<T> input, List<Integer> ksize, List<Integer> strides, String padding) {
   ...
   * @return a new instance of MaxPool3D
   **/
  public static <T> MaxPool3D<T> create(Scope scope, Operand<T> input, List<Integer> ksize, List<Integer> strides, String padding, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("MaxPool3D", scope.makeOpName("MaxPool3D"));
    opBuilder.addInput(input.asOutput());
    long[] ksizeArray = new long[ksize.size()];
    for (int i = 0; i < ksizeArray.length; ++i) {
