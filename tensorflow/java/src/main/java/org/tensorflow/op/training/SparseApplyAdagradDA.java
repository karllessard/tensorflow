...
public final class SparseApplyAdagradDA<T> extends PrimitiveOp implements Operand<T> {
   ...
   **/
  public static <T, U> SparseApplyAdagradDA<T> create(Scope scope, Operand<T> var, Operand<T> gradientAccumulator, Operand<T> gradientSquaredAccumulator, Operand<T> grad, Operand<U> indices, Operand<T> lr, Operand<T> l1, Operand<T> l2, Operand<Long> globalStep) {
   ...
   * @return a new instance of SparseApplyAdagradDA
   **/
  public static <T, U> SparseApplyAdagradDA<T> create(Scope scope, Operand<T> var, Operand<T> gradientAccumulator, Operand<T> gradientSquaredAccumulator, Operand<T> grad, Operand<U> indices, Operand<T> lr, Operand<T> l1, Operand<T> l2, Operand<Long> globalStep, Options options) {
    OperationBuilder opBuilder = scope.graph().opBuilder("SparseApplyAdagradDA", scope.makeOpName("SparseApplyAdagradDA"));
    opBuilder.addInput(var.asOutput());
    opBuilder.addInput(gradientAccumulator.asOutput());
    opBuilder.addInput(gradientSquaredAccumulator.asOutput());
