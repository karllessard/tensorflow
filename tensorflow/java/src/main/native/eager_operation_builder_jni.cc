...
JNIEXPORT void JNICALL Java_org_tensorflow_EagerOperationBuilder_setAttrString(
    JNIEnv* env, jclass clazz, jlong op_handle, jstring attr_name, jbyteArray value) {
...
JNIEXPORT void JNICALL Java_org_tensorflow_EagerOperationBuilder_setAttrStringList(
...
#define DEFINE_SET_ATTR_SCALAR(name, jtype, ctype)                           \
  JNIEXPORT void JNICALL Java_org_tensorflow_EagerOperationBuilder_setAttr##name( \
      JNIEnv* env, jclass clazz, jlong op_handle, jstring attr_name, jtype value) {  \
    static_assert(                                                           \
...
JNIEXPORT void JNICALL Java_org_tensorflow_EagerOperationBuilder_setAttrTensor(
    JNIEnv* env, jclass clazz, jlong handle, jstring attr_name, jlong tensor_handle) {
...
JNIEXPORT void JNICALL Java_org_tensorflow_EagerOperationBuilder_setAttrShape(
    JNIEnv* env, jclass clazz, jlong op_handle, jstring attr_name, jlongArray shape,
    jint num_dims) {
  TFE_Op* op = requireOp(env, op_handle);
  if (op == nullptr) return;
  ...
  const char* cname = env->GetStringUTFChars(attr_name, nullptr);
  TF_Status* status = TF_NewStatus();
  TFE_OpSetAttrShape(op, cname, cvalue.get(), static_cast<int>(num_dims), status);
  throwExceptionIfNotOK(env, status);
  env->ReleaseStringUTFChars(attr_name, cname);
}

JNIEXPORT void JNICALL Java_org_tensorflow_EagerOperationBuilder_setAttrShapeList(
    JNIEnv* env, jclass clazz, jlong op_handle, jstring attr_name, jlongArray shapes,
    jintArray num_dims) {
  TFE_Op* op = requireOp(env, op_handle);
  if (op == nullptr) return;
  ...
  const char* cname = env->GetStringUTFChars(attr_name, nullptr);
  TF_Status* status = TF_NewStatus();
  TFE_OpSetAttrShapeList(op, cname, cdims.get(), cnum_dims.get(), num_dims_length, status);
  env->ReleaseStringUTFChars(attr_name, cname);
}
